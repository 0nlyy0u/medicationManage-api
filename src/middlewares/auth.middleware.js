...
const isAuthorized = async (req, res, next) => {
    ...
    try {
        ...
        const decoded = await JwtProvider.verifyoken(env.ACCESS_TOKEN_PRIVATE_KEY, clientAccessToken)

        const user = await AuthModel.findOneById(decoded._id)
        if (!user) {
            return res.status(HttpStatusCode.UNAUTHORIZED).json({ stt: false, msg: 'Account not found' })
        }
        if (!user.currentAccessToken.includes(clientAccessToken)) {
            return res.status(HttpStatusCode.UNAUTHORIZED).json({ stt: false, msg: 'Access token not found' })
        }
...
const isRefreshToken = async (req, res, next) => {
    ...
    try {
        //Thực hiện giải mã token xem có hợp lệ hay ko
        const decoded = await JwtProvider.verifyoken(env.REFRESH_TOKEN_PRIVATE_KEY, clientRefreshToken)
        ...
        if (!user.currentRefreshToken.includes(clientRefreshToken)) {
            return res.status(HttpStatusCode.UNAUTHORIZED).json({ stt: false, msg: 'Refresh token not found' })
        }
        //Token hợp lệ - Lưu thông tin giải mã được vào req, để sử dụng cho các phần cần xử lý
        req.jwtDecoded = decoded
        // Cho phép REQUEST đi tiếp
