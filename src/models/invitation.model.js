...
const validateSchema = async (data) => {
    return await invitationCollectionSchema.validateAsync(data, {
        abortEarly: false,
    })
}
...
const findOneById = async (id) => {
    try {
        const result = await getDB()
            .collection(invitationCollectionName)
            ...
            .aggregate([
                ...
                },
                {
                    $lookup: {
                        ...
                        from: AuthModel.userCollectionName,
                ...
                },
                {
                    $lookup: {
                        from: AuthModel.userCollectionName,
                        localField: 'boardInvitation.boardId', // array of ObjectIds
                        foreignField: '_id',
                        as: 'board',
                    },
                },
                {
                    $lookup: {
                        from: ClinicModel.boardCollectionName,
                        localField: 'inviteeId', // array of ObjectIds
                        foreignField: '_id',
                        as: 'invitee',
                        pipeline: [
                            // Loại những trường không muốn lấy
                            {
                                $project: {
                                    password: 0,
                                    verifyToken: 0,
